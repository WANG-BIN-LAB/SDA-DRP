...
def main(args, drug, params_dict):
    ...
    # ====================================================
        ...
        # --- Visualization Logic (UPDATED for TransferNetwork signature) ---
        """
        ...
        for i, batch in enumerate(train_labeled_ccle):
            ...
            # UPDATED: No alpha parameter, returns (class_output, feature)
            _, s_feat = network(s_x, node_x, edge_index)
            s_all.append(s_feat)
            s_types.append(s_ty)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--alph', dest='alph', nargs='?', type=float, default=0.2, help='Coefficient of transfer loss (Unused in TSB by default)')
