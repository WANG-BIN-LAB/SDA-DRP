...
def extract_3d_features(pdb_path):
    ...
    try:
        ...
        try:
            ...
            max_charge = np.max(charges) if charges else 0
            min_charge = np.min(charges) if charges else 0
        except:
            avg_charge = 0
            max_charge = 0
            min_charge = 0
        ...
        features = np.array([volume, surface_area, num_atoms, num_bonds, num_rings, 
                           avg_charge, max_charge, min_charge, tpsa, mol_wt])
        

        features_64 = np.zeros(64)
        features_64[:len(features)] = features
        
 
        features_64 = (features_64 - np.mean(features_64)) / (np.std(features_64) + 1e-8)
        
        return features_64
    except Exception as e:
        print(f"Error extracting 3D features: {e}")
        return np.zeros(64)
...
def get_ccle_multi_labeled_dataloader(gex_features_df, batch_size, drug, seed, measurement,
    ...
    try:
        ...
        drug_smiles_df = pd.read_csv(config.gdsc_drugs, header=None)

        for idx, row in drug_smiles_df.iterrows():
            if len(row) >= 2:
                drug_name = str(row[0]).lower()  
                drug_id = str(row[1])  
                drug_id_map[drug_name] = drug_id
        print(f"Loaded drug ID mapping for {len(drug_id_map)} drugs")
    except Exception as e:
        print(f"Error loading drug_smiles.csv: {e}")
        print("Will use drug names as IDs")
    
    drug_3d_feats = []
    for drug_name in drugs_to_keep:
        if drug_name in drug_id_map:
            drug_id = drug_id_map[drug_name]
        else:
            drug_id = drug_name
            print(f"Drug ID not found for {drug_name}, using drug name as ID")
        
        pdb_path = f"3D/{drug_id}.pdb"
        if os.path.exists(pdb_path):
            feat_3d = extract_3d_features(pdb_path)
        else:
            print(f"PDB file not found for drug {drug_name} (ID: {drug_id})")
            feat_3d = np.zeros(64)
        drug_3d_feats.append(feat_3d)
    drug_3d_feats = np.array(drug_3d_feats)
    
    fused_feats = np.concatenate([fp_list, drug_3d_feats], axis=1)
    config.drug_feat = fused_feats
